...
O Nowcast Eleitoral 2026 é um exercício de risco político por unidade da federação, construído sobre três pilares: (i) o resultado do primeiro turno presidencial de 2022 como linha de base; (ii) uma hipótese de *swing nacional uniforme* derivada de pesquisas agregadas; (iii) ajustes regionais e estaduais calibráveis quando há informação adicional. O dashboard interativo permite simular cenários e observar como a recomposição das margens muda a classificação das UFs em cinco categorias (sólido esquerda, provável esquerda, competitivo, provável direita, sólido direita).

A validação do modelo é feita por um *backtest 2018 → 2022*: aplicamos a mesma fórmula ao passado e medimos o erro absoluto médio por UF. Resultado: **MAE = 10,0 pontos percentuais**. O modelo captura corretamente o nível agregado da disputa, mas falha sistematicamente em capturar idiossincrasias regionais — Norte virou pró-Lula muito além do swing nacional, Roraima virou na direção contrária. Esse erro residual é, por construção, a justificativa para os sliders de ajuste regional do dashboard.
...
Quando o usuário ativa o toggle "Usar swing das pesquisas", o modelo substitui `s` por `Δ_pesquisas`. O insumo `M_pesquisas` é a mediana das margens (esquerda − direita) das pesquisas no recorte temporal padrão de 30 dias, lida do arquivo `pesquisas_manual.json`.

A mediana é preferida à média por robustez a outliers (institutos com viés sistemático).
...
Aplicamos a equação central usando 2018 como linha de base e o swing nacional observado entre os dois pleitos:

> `m̂^{2022}_i = m^{2018}_i + Δ^{obs}_nac`
> com `Δ^{obs}_nac = m^{2022}_nac − m^{2018}_nac = 5,2 − (−16,7) = +21,9 pp`

Comparamos `m̂^{2022}_i` com `m^{2022}_i` observado. Métricas de erro:

> `MAE = (1/N) · Σ_i |m^{2022}_i − m̂^{2022}_i|`
> `RMSE = √[(1/N) · Σ_i (m^{2022}_i − m̂^{2022}_i)²]`

### 7.2. Resultados

| Métrica | Valor |
|---|---|
| Swing nacional observado | +21,9 pp |
| Erro médio absoluto (MAE) | 10,0 pp |
| Raiz do erro quadrático médio (RMSE) | 11,9 pp |
| Erro máximo | 26,1 pp (Acre) |
| UFs com swing acima do nacional (mais à esquerda) | 15 |
| UFs com swing abaixo do nacional (mais à direita) | 12 |

Os cinco maiores erros absolutos:

| UF | 2018 | 2022 real | Modelo | Erro |
|---|---|---|---|---|
| AC | −49 | −1 | −27,1 | +26,1 |
| AM | −23 | +20 | −1,1 | +21,1 |
| AP | −37 | +3 | −15,1 | +18,1 |
...
- [x] Projeção de 2º turno parametrizada.
- [x] Decomposição regional da margem nacional.
- [x] Card de pesquisas alimentado por JSON manual.

**V3 (próximos blocos):**
- [ ] Multi-backtest: incluir 2010 → 2014 e 2014 → 2018.
- [ ] Swing por região em vez de swing nacional como baseline.
