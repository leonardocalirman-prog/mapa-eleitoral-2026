...
Transparência metodológica: toda decisão de modelagem é explícita e auditável.
Reatividade: o usuário pode simular cenários movendo parâmetros e observar o resultado em tempo real.
Honestidade sobre incerteza: o backtest mostra o erro real da abordagem, sem mascarar limitações.
...
Faixa de m̂_iClasse> +20Sólido esquerda+5 a +20Provável esquerda−5 a +5Competitivo−20 a −5Provável direita< −20Sólido direita
...
RegiãoMargem 2018 (esq ampla)Margem 2022Swing observadoSudeste−22,0−4,9+17,1 ppCentro-Oeste−27,4−15,9+11,5 ppSul−28,7−17,8+10,9 ppNordeste+39,8+39,8+0,0 ppNorte+2,5+1,6−0,8 pp
...
7.2. Resultados
MétricaModelo A (nacional)Modelo B (regional)Ganho de BErro médio absoluto (MAE)8,6 pp4,7 pp−3,9 pp (−45%)Raiz do erro quadrático médio (RMSE)11,2 pp9,5 pp−1,7 ppErro máximo35,0 pp (Sergipe)44,5 pp (Sergipe)—
...
UF20182022 realA est.A erroB est.B erroSE−22,9+34,7−0,9+35,6−6,1+40,7RR−45,1−46,5−23,2−23,4−37,3−9,2MA+37,0+42,8+58,9−16,1+53,8−11,0RO−41,9−35,4−19,9−15,4−34,1−1,3PA+5,2+12,0+27,1−15,2+13,0−1,1
...
V2 (concluída):
 ...
 Dashboard interativo Quarto + Observable.
 Backtest 2018 → 2022 com decomposição por UF.
 Modelo B (swing regional) — MAE 6,9 pp, ganho de 31% sobre Modelo A.
 Toggle "Aplicar heterogeneidade regional" no Simulador.
 Projeção de 2º turno parametrizada (slider migOutros).
 Decomposição regional da margem nacional.
 Card de pesquisas alimentado por JSON manual (12 institutos, mediana +7,1 pp).
 Convenção visual brasileira: vermelho = esquerda, azul = direita.
 Publicação automatizada via GitHub Actions + Pages.
...
Nome no códigoNotaçãoTipoSignificadoExemploresultados[i].margem_2022m^{2022}_iconstanteMargem real Lula − Bolso por UF em 2022 (pp)BA: +56, SP: −12resultados[i].lula_2022l^{2022}_iconstante% Lula por UF em 2022BA: 73, SP: 36resultados[i].bolso_2022b^{2022}_iconstante% Bolsonaro por UF em 2022BA: 17, SP: 48resultados[i].votos_validosv_iconstanteVotos válidos por UF em 2022 (peso)SP: 22,4 mimargem_2022_nacionalm^{2022}_nacconstanteMargem nacional 1T 2022+5,2 ppswingNacs (slider)parâmetroSwing nacional do usuáriodefault: 0usarPesquisa(toggle)parâmetroSubstituir slider por swing das pesquisasdefault: falsomigOutrosf (slider)parâmetroFração dos "Outros" pra esquerda no 2Tdefault: 0,60ajN, ajNE, ajCO, ajSE, ajSa^R_iparâmetroAjuste extra por macrorregiãodefault: 0 cadaswingEfetivo—derivadaSwing efetivamente aplicadocalculadamargens[i].margem_estimadam̂_iderivadaMargem estimada por UFcalculadamargens[i].classe—derivadaClasse de competitividade (5 faixas)"competitivo"counts—derivadaContagem de UFs por classe{solido_esq: 10, ...}resumo_nacional.lulaL̂_nacderivada% Lula nacional estimado46,2%resumo_nacional.margem_nacionalM̂_nacderivadaMargem nacional estimada+5,3 ppprojecao_2t.margemM̂^{2T}derivadaMargem nacional do 2T+7,9 ppdecomposicao_regional[R].contribuicao_ppc_RderivadaContribuição da região para M̂_nacvariapesquisas.margemM_pesquisasexternaMediana das pesquisasvariabacktest.mae_ppMAEexternaErro absoluto médio do backtest10,0 pp
...
Editar data/pesquisas_manual.json adicionando pesquisas novas.
Rodar python prep_data.py.
Verificar localmente com quarto preview.
Commit + push. GitHub Actions republica em ~3 minutos.
